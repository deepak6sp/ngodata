<?php
require_once '../model/UserVO.class.php';
require_once '../model/UserQO.class.php';
require_once '../model/UserDAO.class.php';
require_once '../model/UserValidationHandler.class.php';
...
/**
 * Factory: User
 * This is the factory for User
 * - this creates and sets User objects.
...
class UserFactory {
	 ...
	 * Method created automatically by NGOData system
	 */
	public function createUser(iPostData $data) {
		echo '<pre>';
		print 'in createUser: ';
		echo '</pre>';
		...
		$vo->setData($data);
		$vo->setQueryObject($qo);
		$dao = new UserDAO($this->getSQLFactory());
		$dao->setValueObject($vo);
		return $dao;
	}
	...
	public function validate($validationArray) {
		...
		//username needs validating.
		//required are required tests.
		//password needs validating.
		//required are required tests.
	 ...
	 * - Assumes $obj exists in persistent storage.
	 */
	public function doDelete(UserDAO $obj) {
		// do some magic...
